...
open Lean
open Lean.Json
open SemiAlgebraic.Computational.RealConstraints
...
private def usage : String :=
  String.intercalate "\n"
    ...
    , "  --backend z3|yices: docker run semialgebraic/smt-solvers:latest python3 scripts/cad_smt_solve.py"
    ]
...
private def runSMTDocker (repo : System.FilePath) (inAbs : System.FilePath) (solver : String) : IO String := do
  ...
  let image := "semialgebraic/smt-solvers:latest"
...
def main (argv : List String) : IO Unit := do
  ...
  let jcs := SemiAlgebraic.Util.JCS.canonicalString problemJson
  let sha ← SemiAlgebraic.Util.sha256HexOfStringIO jcs
  ...
  let certJson :=
    Json.mkObj <|
      ...
      , ("docker_image", Json.str (match backend with | "dreal" => "dreal/dreal4:latest" | "z3" | "yices" => "semialgebraic/smt-solvers:latest" | other => other))
      , ("result", solverJson)
      , ("witness", witness)
